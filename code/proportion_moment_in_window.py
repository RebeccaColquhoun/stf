...
combined.columns = ['event', 'scardec', 'ye', 'isc', 'sigloch', 'usgs', 'mag']
for max_depth_lim in [30, 70, 100, 300, 6378]:
	for min_depth_lim in [0, 30, 70, 100, 300]:
		...
		for time in [1, 2, 3, 4, 5, 10]:
			...
			for dataset in unique_datasets:
				...
				plt.scatter(subset['simpson'], subset['simpson_short']/subset['simpson'], label=dataset, alpha=0.5, facecolors='none', edgecolors=dataset_colors[dataset])
			...
			plt.ylabel(f'Proportion of moment released in first {time} seconds')
			...
			plt.legend()
			plt.savefig(f'/home/earthquakes1/homes/Rebecca/phd/stf/figures/proportion_moment_in_absolute_time/proportion_moment_in_{time}_seconds_min_depth_{min_depth_lim}_max_depth_{max_depth_lim}.png')
			plt.close()
			...
			for i, dataset in enumerate(unique_datasets_2):
				...
				if dataset != 'usgs':
					subset_no_nan = subset.dropna()
					subset = subset_no_nan[abs(subset_no_nan['magnitude']-2/3*(np.log10(subset_no_nan['simpson'])-9.1)) < 1]
				ax.scatter(subset['simpson'], subset['simpson_short']/subset['simpson'], label=dataset, alpha=0.7, facecolors='none', edgecolors=dataset_colors[dataset.split('_')[0]])
				#ax.set_xlim(5, 10)
				#ax.set_yscale('log')
				ax.set_xscale('log')
				...
				if i%3 == 0:
					ax.set_ylabel(f'Proportion of moment released in first {time} seconds')
				if i//3 == 1:
					ax.set_xlabel('Magnitude')
			fig.suptitle(f'{min_depth_lim} < Depth < {max_depth_lim} km')
			plt.tight_layout()
			plt.savefig(f'/home/earthquakes1/homes/Rebecca/phd/stf/figures/proportion_moment_in_absolute_time/proportion_moment_in_{time}_seconds_dataset_min_depth_{min_depth_lim}_max_depth_{max_depth_lim}.png', dpi=300, bbox_inches='tight')
			plt.close()
