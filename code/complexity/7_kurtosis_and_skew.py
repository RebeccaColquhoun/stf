...
for i, row in combined.iterrows():
	...
	for dataset, get_stf in zip(['scardec_opt', 'scardec_moy', 'ye', 'usgs', 'sigloch', 'isc'], [get_scardec_stf, get_scardec_stf, get_ye_stf, get_usgs_stf, get_sigloch_stf, get_isc_stf]):
		...
		for time, momentrate in zip(times_to_use, momentrates_to_use):
			try:
				...
				# plt.show()
				# break
				kurtosis = scipy.stats.kurtosis(interp_momentrate)
				skew = scipy.stats.skew(interp_momentrate)

				event_dict[name_to_save + '_kurtosis'] = kurtosis
				event_dict[name_to_save + '_skew'] = skew
	...
	if i % 100 == 0:
		...
		print(df)
		df.to_csv(f'{output_dir}kurtosis_skew.csv', header=False, mode='a')
		peaks_dict = {}
