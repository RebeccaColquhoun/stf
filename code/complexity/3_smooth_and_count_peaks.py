...
smoothing_param = 20 # 1/smoothing_param is the fraction of the length of the moment rate to smooth over
r2_limit = 0.95
...
for i, row in combined.iterrows():
	...
	for dataset, get_stf in zip(['scardec_opt', 'scardec_moy', 'ye', 'usgs', 'sigloch', 'isc'], [get_scardec_stf, get_scardec_stf, get_ye_stf, get_usgs_stf, get_sigloch_stf, get_isc_stf]):
		...
		for time, momentrate in zip(times_to_use, momentrates_to_use):
			try:
				...
				peaks, properties = find_peaks(momentrate, height=height_threshold*max(momentrate), distance=100)
				num_peaks = len(peaks)
				event_dict[name_to_save + '_peaks'] = num_peaks
	...
	if i % 100 == 0:
		...
		print(df)
		df.to_csv(f'{output_dir}peaks_dict.csv', header=False, mode='a')
		peaks_dict = {}
