...
obs_x = np.concatenate((np.zeros(100), obs_x))
obs_y = np.concatenate((np.zeros(100), obs_y))

plot_x = np.linspace(-0.01, 1, 10100)# x #np.concatenate((np.zeros(100), x))
plot_trap_y = np.concatenate((np.zeros(100), trap_y))
plot_triangle_y = np.concatenate((np.zeros(100), triangle_y))
plot_gaussian_y = np.concatenate((np.zeros(100), gaussian_y))
plot_sine_boxcar_y = np.concatenate((np.zeros(100), sine_boxcar_y))
plot_boxcar_y = np.concatenate((np.zeros(100), boxcar_y))

plt.plot(obs_x, obs_y, label = 'Average observed', color = 'k')

plt.plot(plot_x, plot_trap_y, label = f'Trapezium - {np.sum((plot_trap_y - obs_y)**2):.2f}', linestyle=styles.pop(0), linewidth = 2.5)
plt.plot(plot_x, plot_triangle_y, label = f'Triangle - {np.sum((plot_triangle_y - obs_y)**2):.2f}', linestyle=styles.pop(0), linewidth = 2.5)
plt.plot(plot_x, plot_gaussian_y, label = f'Gaussian - {np.sum((plot_gaussian_y - obs_y)**2):.2f}', linestyle=styles.pop(0), linewidth = 2.5)
plt.plot(plot_x, plot_sine_boxcar_y, label = f'Sine Boxcar - {np.sum((plot_sine_boxcar_y - obs_y)**2):.2f}', linestyle=styles.pop(0), linewidth = 2.5)
plt.plot(plot_x, plot_boxcar_y, label = f'Boxcar - {np.sum((plot_boxcar_y - obs_y)**2):.2f}', linestyle=styles.pop(0), linewidth = 2.5)
...
plt.savefig(f'/home/earthquakes1/homes/Rebecca/phd/stf/figures/average_STF/average_observed_and_models_all_v3.png')
plt.close()

with open(f'/home/earthquakes1/homes/Rebecca/phd/stf/figures/average_STF/average_best_fitting_models_v3.txt', 'a') as f:
	f.write(f'ALL \n')
	f.write(f'Gaussian \n')
	f.write(f'height: {best_gauss_height}, mean: {best_gauss_mean}, std: {best_gauss_std}\n')
	f.write(f'2-norm {np.sum((plot_gaussian_y - obs_y)**2):.2f}\n')
	f.write(f'Triangle \n')
	f.write(f'center: {best_tri_center}, height: {best_tri_height}, start: {best_tri_start}, end: {best_tri_end}\n')
	f.write(f'2-norm {np.sum((plot_triangle_y - obs_y)**2):.2f}\n')
	f.write(f'Boxcar \n')
	f.write(f'start: {best_box_start}, end: {best_box_end}, height: {best_box_height}\n')
	f.write(f'2-norm {np.sum((plot_boxcar_y - obs_y)**2):.2f}\n')
	f.write(f'Trapezium \n')
	...
	f.write(f'2-norm {np.sum((plot_trap_y - obs_y)**2):.2f}\n')
	...
	f.write(f'2-norm {np.sum((plot_sine_boxcar_y - obs_y)**2):.2f}\n')
	f.write('\n\n\n')
