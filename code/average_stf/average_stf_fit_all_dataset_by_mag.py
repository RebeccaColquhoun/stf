...
for i, b in enumerate(boundaries):
	print(f'Magnitude {b} to {b+interval}')
	...
	for left_flat in np.linspace(0., 1, points_to_test):
		for right_flat in np.linspace(0., 1, points_to_test):
			for height in np.linspace(0, 1, points_to_test):
				for start_height in np.linspace(0, 1, points_to_test):
					for end_height in np.linspace(0, 1, points_to_test):
						for ramp_right in np.linspace(0.0, 1, points_to_test):
							for end_ramp_height in np.linspace(0, 1, points_to_test):
								try:
	...
	plt.savefig(f'/home/earthquakes1/homes/Rebecca/phd/stf/figures/average_STF/average_observed_and_models_all_v2_magnitude_{b}_{b+interval}.png')
	plt.close()

	with open(f'/home/earthquakes1/homes/Rebecca/phd/stf/figures/average_STF/average_best_fitting_models_by_mag.txt', 'a') as f:
		f.write(f'M{b}--{b+interval} \n')
