...
for dataset_wanted in ['scardec_opt', 'scardec_moy', 'ye', 'usgs', 'sigloch', 'isc']:
	...
	ks = scipy.stats.ks_2samp(trap_y, obs_y)
	plt.plot(x, gaussian_y, label = f'Gaussian - {ks:.2f}', linestyle=styles.pop(0), linewidth = 2.5)
	...
	plt.plot(x, triangle_y, label = f'Triangle - {ks:.2f}', linestyle=styles.pop(0), linewidth = 2.5)

	boxcar_y = boxcar(x, start = best_box_start, end = best_box_end, peak = best_box_height)
	ks = scipy.stats.ks_2samp(trap_y, obs_y)
	plt.plot(x, boxcar_y, label = f'Boxcar - {ks:.2f}', linestyle=styles.pop(0), linewidth = 2.5)
	...
	ks = scipy.stats.ks_2samp(trap_y, obs_y)
	plt.plot(x, trap_y, label = f'Trapezium - {ks:.2f}', linestyle=styles.pop(0), linewidth = 2.5)
	...
	plt.plot(x, sine_boxcar_y, label = f'Sine Boxcar - {ks:.2f}', linestyle=styles.pop(0), linewidth = 2.5)

	plt.ylabel('Normalized Moment Rate')
	plt.xlabel('Normalized Time')
	plt.legend()

	plt.savefig(f'/home/earthquakes1/homes/Rebecca/phd/stf/figures/average_STF/average_observed_and_models_{dataset_wanted}_v2.png')
	plt.close()

	with open(f'/home/earthquakes1/homes/Rebecca/phd/stf/figures/average_STF/average_best_fitting_models.txt', 'a') as f:
		f.write(f'{dataset_wanted}\n')
		f.write(f'Gaussian \n')
