	...
	for min_depth_lim in [0]:#, 30, 70]:
		...
		for time in [1]:#, 2, 3, 4, 5, 10]:
			...
			for dataset in unique_datasets:
				...
				print(np.count_nonzero(ratio > 0.99))
				print(len(subset))
			...
			plt.legend()
			plt.savefig(f'/home/earthquakes1/homes/Rebecca/phd/stf/figures/moment_in_absolute_time/ratio_in_{time}_seconds_min_depth_{min_depth_lim}_max_depth_{max_depth_lim}_no_log.png')
			plt.close()
			...
			fig, axes = plt.subplots(nrows=3, ncols=2, figsize=(5, 7), sharey=True, sharex=True)
			...
			plt.tight_layout()
			plt.savefig(f'/home/earthquakes1/homes/Rebecca/phd/stf/figures/moment_in_absolute_time/ratio_in_{time}_seconds_dataset_min_depth_{min_depth_lim}_max_depth_{max_depth_lim}_no_log.png')
			plt.close()
			#plt.show()